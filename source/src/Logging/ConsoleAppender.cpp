...
namespace Logging
{
	ConsoleAppender::ConsoleAppender(const fwstr name)
		: AppenderBase(name)
	{ }
	...
	fwvoid ConsoleAppender::DoAppend(LogData data)
	{
		std::stringstream ss;
	...
	fwvoid ConsoleAppender::ColoredOutput(fwstr text, CONSOLE_COLORS foreground)
	{
		/*
		enum {
		BLACK             = 0,
