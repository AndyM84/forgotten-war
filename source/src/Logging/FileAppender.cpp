...
namespace Chimera
{
	namespace Logging
	{
		FileAppender::FileAppender(const cxstring name)
			: AppenderBase(name)
		{
		...
		FileAppender::FileAppender(const cxstring name, const cxstring file)
			: AppenderBase(name)
		{
		...
		cxvoid FileAppender::DoAppend(LogData data)
		{
			std::stringstream ss;
		...
		cxvoid FileAppender::OutputToFile(cxstring line)
		{
			if (!this->m_LogFile.is_open())
			{
				return;
